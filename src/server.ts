...
function assertEnv(key: keyof typeof env, ...additionalChecks: ((input: (typeof env)[typeof key]) => string)[]) {
	if (!env[key]) {
...
assertEnv("MIDI_CHANNEL", (c: number) => (c < 0 || c > 15) && "Value must be between 0 and 15");
...
if (env.SERVER_ENABLE) {
	...
	app.use(compression({ threshold: 0 }), sirv("static", { dev }), bodyParser.json());
...
studioliveService.connect({ host: env.CONSOLE_HOST, port: env.CONSOLE_PORT }).then(() => {
	if (env.SERVER_WEBMIDI_EXCLUSIVE) {
		...
		const connectedDevice = connectMidiDevice(env.MIDI_DEVICE);
		...
		studioliveService.withClient((client) => {
			try {
				midiService.connect(client, connectedDevice, FaderPortManager, {
					s: '',
					test: store,
				});
			} catch (e) {
