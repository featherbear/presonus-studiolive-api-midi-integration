// If i press a button, should i show early feedback?
// Or just wait for the console to respond?
// But what about if i'm not connected to the console?
...
import {
	...
	VELOCITY,
	VALUE_BAR_MODE,
} from "./interface/vendorConstants";
...
export default class FaderPortManager extends FaderPortDevice implements DeviceManager<FaderPortConfig> {
	...
	constructor(midiDevice: MidiDeviceGroup, config: FaderPortConfig) {
		...
		// Does this talk to the API?
		setTimeout(() => {
			console.log("Calling refresh visible channels");
			this.refreshVisibleChannels();
		}, 1000);
	}

	disconnectAPI() {
		const _apiRegistrations = [...this.APIregistrations];
		this.APIregistrations = [];

		if (this.isConnected) {
			for (const [evt, fn] of _apiRegistrations) {
				this.API.removeListener(<any>evt, <any>fn);
			}
		}
	}
	connectAPI(api: Client) {
		this.disconnectAPI();

		const registerFuncs: (keyof Client)[] = ["on", "once", "addListener"];
		this.API = new Proxy(api, {
			get: (target, prop, receiver) => {
				if (registerFuncs.includes(<any>prop)) {
					return (...args: [any, any]) => {
						this.APIregistrations.push(args);
		...
		this.API.on("level", (evt) => this.tryNotifyFader(evt.channel, evt.level));

		this.API.on("mute", (evt) => this.tryNotifyMute(evt.channel, evt.status));

		this.API.on("solo", (evt) => this.tryNotifySolo(evt.channel, evt.status));
		...
		// this.refreshVisibleChannels()
		// }, 1000)
		// TODO: Do an update to get latest state?
	}
	...
	handleMidiMessage(message: MidiMessage) {
		// TODO: perhaps we have a device mode
		// momentary, latching, game.

		if (!this.isConnected) {
			console.log("not connect lol");
			// API is not connected, flashy flashy
